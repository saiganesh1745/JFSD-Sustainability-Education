...
const LessonManager = () => {
    ...
    const handleSaveLesson = async (e) => {
        ...
        try {
            if (editingLesson) {
                await axios.put(`http://localhost:8080/admin/lessons/${editingLesson.id}`, newLesson);
            } else {
                await axios.post('http://localhost:8080/admin/lessons', newLesson);
            }
            resetForm();
            fetchLessons();
        } catch (error) {
    ...
    const handleDeleteLesson = async (id) => {
        try {
            await axios.delete(`http://localhost:8080/admin/lessons/${id}`);
            fetchLessons();
        } catch (error) {
